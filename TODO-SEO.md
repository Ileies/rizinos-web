...
- [ ] **OG-Image erstellen** - `socialcard.jpeg` (1200x630px) fehlt im static-Verzeichnis; wird in og:image bereits referenziert aber nicht ausgeliefert
...
- [ ] **`<html lang>` dynamisch setzen** - Aktuell hardcoded `lang="de"` in beiden `app.html`; bei aktivem Cookie-basiertem Locale-Switching sollte das dynamisch gesetzt werden
...
- [ ] **Sitemap dynamisch generieren** - `src/routes/sitemap.xml/+server.ts` statt statischer Datei, damit Blog-Posts und neue Seiten automatisch enthalten sind
