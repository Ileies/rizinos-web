<script lang="ts">
	import { cookie_banner_text, cookie_accept, cookie_decline, cookie_privacy_link } from '$lib/messages.svelte';
...
<div class="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background/95 backdrop-blur-sm">
	<div class="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-2.5 text-sm">
		<p class="text-muted-foreground">
			{cookie_banner_text()}
			<a href="/privacy" class="underline underline-offset-2 hover:text-foreground">{cookie_privacy_link()}</a>
		</p>
