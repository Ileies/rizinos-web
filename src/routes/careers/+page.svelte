<script lang="ts">
	import Container from '$ui/homepage/Container.svelte';
</script>

<svelte:head>
	<title>Careers - RizinOS</title>
	<meta name="description" content="Work on RizinOS." />
</svelte:head>

<Container>
	<div class="py-24 sm:py-32">
		<p class="text-primary mb-4 text-sm font-semibold tracking-widest uppercase">Careers</p>
		<h1 class="text-foreground mb-6 text-4xl font-black tracking-tight sm:text-5xl">
			Join the team.
		</h1>
		<p class="text-muted-foreground max-w-2xl text-lg leading-relaxed">
			Open positions will be listed here when available.
		</p>
	</div>
</Container>
